...
public class RepositoryResource {
    ...
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON)
    @Operation(summary = "Request update existing repository")
    @APIResponse(name = "RefreshRepository", responseCode = "202", description = "Accepted")
    @APIResponse(responseCode = "404", description = "Doesn't exist.", content = @Content(mediaType = APPLICATION_JSON, schema = @Schema(implementation = CatalogError.class, required = true)))
    public Response update(@Parameter(name = "id", description = "internal id") @PathParam("id") Long id)
            throws RepositoryNotFoundException {
        repositoryService.updateRepositoryRequest(id);
        return Response.accepted().build();
    }
